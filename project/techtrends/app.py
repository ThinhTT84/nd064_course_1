...
def get_total_posts():
    connection = get_db_connection()
    row = connection.execute('SELECT count(1) as total FROM posts').fetchone()
    connection.close()
    return row['total']
...
def metrics():
    ...
    response = app.response_class(
        #TODO: need to find solution for connection count as it should share between multiple requests. 
        response=json.dumps({"post_count": total, "db_connection_count": 1}),
        status=200,
        mimetype='application/json'
    )
    return response
